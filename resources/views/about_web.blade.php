<!DOCTYPE html>
<html lang="en">

    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>bry/fks</title>

        <!-- Icon -->
        <link rel="shortcut icon" href="assets/favicon.ico" type="image/x-icon">

        <!-- CSS link(s) -->
        <link rel="stylesheet" href="css/global.css">
        <link rel="stylesheet" href="css/aboutweb.css">

        <!-- AOS CSS -->
        <link rel="stylesheet" href="css/aos.css">
    </head>

    <body>
        <header>
            <nav-bar/>
        </header>
        <main>
            <!-- About Web -->
            <section id="about-web">
                <div class="section-title">
                    <span data-aos="fade-up" data-aos-once="true">&mdash; About Website</span>
                    <span data-aos="fade-up" data-aos-delay="50" data-aos-once="true">Website History.</span>
                </div>
                <!-- History -->
                <div class="section-content">
                    <p data-aos="fade-up" data-aos-delay="100" data-aos-once="true">
                        The first version of this website was made in 2020. So, there was a project for my English
                        class in 12<sup>th</sup> grade Senior High School, where students should assign a goal to do
                        something and track the progress each days. Therefore, I chose to build a simple personal
                        website, but actually that was not my first time develop a website. I'm very enthusiastic that
                        day, so I swept the project only in 5 working days, from writting the html, css, untill hosted
                        it.
                        <br><br>
                        Looking back to those days, it is really fun and challanging for me, and I think this project is
                        the reason I'm super insterested into the website development, other than tech development, but
                        (maybe) I could change my interest some day, well who knows the future?
                    </p>
                </div>
                <!-- Tech stacks -->
                <div class="tech-stack-wrapper">
                    <span data-aos="fade-up" data-aos-delay="150" data-aos-once="true">Used Techs in Website.</span>
                    <div class="section-content">
                        <p data-aos="fade-up" data-aos-delay="200" data-aos-once="true">
                            There are many tech for website development, either for frontend or backend development.
                            But,
                            I want to keep it simple, so these are tech stacks that I used to build this website (I give
                            a link for each tech too, so feel free to use it).
                        </p>
                        <div class="tech-stack">
                            <!-- Code Editor -->
                            <div data-aos="fade-up" data-aos-delay="250" data-aos-once="true">
                                <span>Code Editor.</span>
                                <ul class="stack">
                                    <!-- VS Code -->
                                    <li>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                            viewBox="0 0 24 24" fill="hsl(205, 77%, 47%)">
                                            <path
                                                d="m21.29 4.1-4.12-2a1.36 1.36 0 0 0-.48-.1h-.08a1.18 1.18 0 0 0-.72.24l-.14.12-7.88 7.19L4.44 7a.83.83 0 0 0-.54-.17.88.88 0 0 0-.53.17l-1.1 1a.8.8 0 0 0-.27.61.84.84 0 0 0 .27.62l3 2.71-3 2.72a.84.84 0 0 0 0 1.23l1.1 1a.89.89 0 0 0 .6.22.93.93 0 0 0 .47-.17l3.43-2.61 7.88 7.19a1.2 1.2 0 0 0 .76.36h.17a1 1 0 0 0 .49-.12l4.12-2a1.25 1.25 0 0 0 .71-1.1V5.23a1.26 1.26 0 0 0-.71-1.13zM17 16.47l-6-4.53 6-4.53z">
                                            </path>
                                        </svg>
                                        <a target="_blank" rel="noreferrer noopener"
                                            href="https://code.visualstudio.com/">Visual
                                            Studio Code</a>
                                    </li>
                                </ul>
                            </div>
                            <!-- Technologies -->
                            <div data-aos="fade-up" data-aos-delay="300" data-aos-once="true">
                                <span>Technologies.</span>
                                <ul class="stack">
                                    <!-- HTML -->
                                    <li>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                            viewBox="0 0 24 24" fill="hsl(12, 78%, 54%)">
                                            <path
                                                d="M4.136 3.012h15.729l-1.431 16.15-6.451 1.826-6.414-1.826-1.433-16.15zm5.266 7.302-.173-2.035 7.533.002.173-1.963-9.87-.002.522 5.998h6.835l-.243 2.566-2.179.602-2.214-.605-.141-1.58H7.691l.247 3.123L12 17.506l4.028-1.08.558-6.111H9.402v-.001z">
                                            </path>
                                        </svg>
                                        <a target="_blank" rel="noreferrer noopener" href="https://html.com/">HTML5</a>
                                    </li>
                                    <!-- CSS -->
                                    <li>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                            viewBox="0 0 24 24" fill="hsl(228, 73%, 51%)">
                                            <path
                                                d="M4.192 3.143h15.615l-1.42 16.034-6.404 1.812-6.369-1.813L4.192 3.143zM16.9 6.424l-9.8-.002.158 1.949 7.529.002-.189 2.02H9.66l.179 1.913h4.597l-.272 2.62-2.164.598-2.197-.603-.141-1.569h-1.94l.216 2.867L12 17.484l3.995-1.137.905-9.923z">
                                            </path>
                                        </svg>
                                        <a target="_blank" rel="noreferrer noopener"
                                            href="https://www.w3.org/Style/CSS/Overview.en.html">CSS3</a>
                                    </li>
                                    <!-- Javascript -->
                                    <li>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                            viewBox="0 0 24 24" fill="hsl(53, 87%, 52%)">
                                            <path
                                                d="M3 3h18v18H3V3zm16.525 13.707c-.131-.821-.666-1.511-2.252-2.155-.552-.259-1.165-.438-1.349-.854-.068-.248-.078-.382-.034-.529.113-.484.687-.629 1.137-.495.293.09.563.315.732.676.775-.507.775-.507 1.316-.844-.203-.314-.304-.451-.439-.586-.473-.528-1.103-.798-2.126-.775l-.528.067c-.507.124-.991.395-1.283.754-.855.968-.608 2.655.427 3.354 1.023.765 2.521.933 2.712 1.653.18.878-.652 1.159-1.475 1.058-.607-.136-.945-.439-1.316-1.002l-1.372.788c.157.359.337.517.607.832 1.305 1.316 4.568 1.249 5.153-.754.021-.067.18-.528.056-1.237l.034.049zm-6.737-5.434h-1.686c0 1.453-.007 2.898-.007 4.354 0 .924.047 1.772-.104 2.033-.247.517-.886.451-1.175.359-.297-.146-.448-.349-.623-.641-.047-.078-.082-.146-.095-.146l-1.368.844c.229.473.563.879.994 1.137.641.383 1.502.507 2.404.305.588-.17 1.095-.519 1.358-1.059.384-.697.302-1.553.299-2.509.008-1.541 0-3.083 0-4.635l.003-.042z">
                                            </path>
                                        </svg>
                                        <a target="_blank" rel="noreferrer noopener"
                                            href="https://www.javascript.com/">Javascript</a>
                                    </li>
                                    <!-- Node.js -->
                                    <li>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                            viewBox="0 0 24 24" fill="hsl(84, 54%, 53%)">
                                            <path
                                                d="M12 21.985c-.275 0-.532-.074-.772-.202l-2.439-1.448c-.365-.203-.182-.277-.072-.314.496-.165.588-.201 1.101-.493.056-.037.129-.02.185.017l1.87 1.12c.074.036.166.036.221 0l7.319-4.237c.074-.036.11-.11.11-.202V7.768c0-.091-.036-.165-.11-.201l-7.319-4.219c-.073-.037-.165-.037-.221 0L4.552 7.566c-.073.036-.11.129-.11.201v8.457c0 .073.037.166.11.202l2 1.157c1.082.548 1.762-.095 1.762-.735V8.502c0-.11.091-.221.22-.221h.936c.108 0 .22.092.22.221v8.347c0 1.449-.788 2.294-2.164 2.294-.422 0-.752 0-1.688-.46l-1.925-1.099a1.55 1.55 0 0 1-.771-1.34V7.786c0-.55.293-1.064.771-1.339l7.316-4.237a1.637 1.637 0 0 1 1.544 0l7.317 4.237c.479.274.771.789.771 1.339v8.458c0 .549-.293 1.063-.771 1.34l-7.317 4.236c-.241.11-.516.165-.773.165zm2.256-5.816c-3.21 0-3.87-1.468-3.87-2.714 0-.11.092-.221.22-.221h.954c.11 0 .201.073.201.184.147.971.568 1.449 2.514 1.449 1.54 0 2.202-.35 2.202-1.175 0-.477-.185-.825-2.587-1.063-1.999-.2-3.246-.643-3.246-2.238 0-1.485 1.247-2.366 3.339-2.366 2.347 0 3.503.809 3.649 2.568a.297.297 0 0 1-.056.165c-.037.036-.091.073-.146.073h-.953a.212.212 0 0 1-.202-.164c-.221-1.012-.789-1.34-2.292-1.34-1.689 0-1.891.587-1.891 1.027 0 .531.237.696 2.514.99 2.256.293 3.32.715 3.32 2.294-.02 1.615-1.339 2.531-3.67 2.531z">
                                            </path>
                                        </svg>
                                        <a target="_blank" rel="noreferrer noopener"
                                            href="https://nodejs.org/en">Node.js</a>
                                    </li>
                                    <!-- Express -->
                                    <li>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                            viewBox="0 0 50 50" fill="hsl(0, 0%, 3%)"
                                            style="background-color: hsl(0, 0%, 100%)">
                                            <path
                                                d="M49.729 11h-.85c-1.051 0-2.041.49-2.68 1.324l-8.7 11.377-8.7-11.377C28.162 11.49 27.171 11 26.121 11h-.85l10.971 14.346L25.036 40h.85c1.051 0 2.041-.49 2.679-1.324L37.5 26.992l8.935 11.684C47.073 39.51 48.063 40 49.114 40h.85L38.758 25.346 49.729 11zM21.289 34.242c-2.554 3.881-7.582 5.87-12.389 4.116C4.671 36.815 2 32.611 2 28.109L2 27h12v0h11l0-4.134c0-6.505-4.818-12.2-11.295-12.809C6.273 9.358 0 15.21 0 22.5l0 5.573c0 5.371 3.215 10.364 8.269 12.183 6.603 2.376 13.548-1.17 15.896-7.256 0 0 0 0 0 0h-.638C22.616 33 21.789 33.481 21.289 34.242zM2 22.5C2 16.71 6.71 12 12.5 12S23 16.71 23 22.5V25H2V22.5z">
                                            </path>
                                        </svg>
                                        <a target="_blank" rel="noreferrer noopener"
                                            href="https://expressjs.com/">Express</a>
                                    </li>
                                    <!-- Swiper -->
                                    <li>
                                        <svg width="24" height="24" viewBox="0 0 129 129"
                                            xmlns="http://www.w3.org/2000/svg">
                                            <path
                                                d="M97.5869496,9.51093626 C115.83804,20.7848256 128,40.9721238 128,64 C128,99.346224 99.346224,128 64,128 C56.2920179,128 48.9022945,126.637372 42.0606106,124.137797 L41.3296807,123.865645 L42.5199148,123.48344 L44.0499006,122.981598 L44.8047554,122.729449 L44.8047554,122.729449 L45.5528525,122.476476 L47.0287766,121.968044 L48.477679,121.456268 L49.8995657,120.941118 L51.2944429,120.422561 L51.9817549,120.161995 L53.3361292,119.638268 C54.7663971,119.077773 56.1505947,118.511327 57.4887554,117.938757 L58.4837445,117.508179 L59.7032082,116.966702 C60.2391997,116.725238 60.7672019,116.482694 61.2872173,116.239057 L62.0612505,115.87278 L63.1998411,115.320271 L63.7590314,115.042615 L63.7590314,115.042615 L64.3114862,114.76402 L65.3961918,114.203997 C74.5017145,109.427674 80.6883363,104.239002 83.9729034,98.5499653 C90.9321253,86.4962393 84.8542511,73.6150855 67.4065187,60.4830885 L65.9866432,59.4185495 L65.1241488,58.7570907 L64.2867532,58.1006716 L63.4743719,57.4492672 L62.6869203,56.8028527 L62.3025167,56.4815088 L62.3025167,56.4815088 L61.9243138,56.161403 L61.1864678,55.5248932 L60.6492814,55.0507377 L60.6492814,55.0507377 L60.1259396,54.5793364 L59.4496246,53.9550673 L58.7977741,53.3356509 L58.1703034,52.7210621 L57.865684,52.4155703 L57.865684,52.4155703 L57.5671278,52.1112761 L56.9881629,51.5062679 C50.0910281,44.1742986 48.550875,37.8976624 51.6821025,32.4742173 C53.275799,29.713854 56.0387768,26.999998 59.9662039,24.3569485 L60.4430842,24.040124 L61.1940488,23.5569049 C61.5336196,23.3425039 61.8809347,23.1285905 62.2359916,22.9151777 L62.7743828,22.5954356 L63.603739,22.1172522 L64.4592124,21.6408141 L65.1179529,21.2846511 L65.1179529,21.2846511 L65.7913778,20.9295026 L66.7121153,20.4575739 C67.4124544,20.1043216 68.1348104,19.7526404 68.8791706,19.4025938 L69.6308651,19.0530944 L70.6559388,18.5888147 L71.1782513,18.3574212 L72.2424237,17.8961475 L72.7842819,17.6662758 L73.887538,17.2080833 L74.4489343,16.9797709 L75.5912592,16.5247346 L76.1721862,16.2980192 L77.353565,15.8462143 L78.5609715,15.3966054 L79.174433,15.1726349 L80.4208685,14.7263826 L81.6933149,14.2824097 L82.9917658,13.8407497 L83.6507409,13.6207974 L84.9881859,13.1826691 L86.3516187,12.7469371 L87.7410327,12.3136347 L89.1564212,11.8827954 L90.5977776,11.4544525 L92.4359804,10.9225852 L94.3147346,10.394736 L96.6227494,9.76671243 L97.5869496,9.51093626 Z M64,0 C71.7830114,0 79.2415381,1.38928435 86.1411179,3.93339103 L86.8782091,4.21038217 L86.4108627,4.35186467 L84.1313124,5.0545346 L83.3848908,5.2905768 L81.9121853,5.76541455 L80.4663248,6.24395126 L79.0473024,6.72622031 L77.6551115,7.21225509 L76.2897456,7.70208898 L75.6171199,7.94844102 L73.9648828,8.56854028 L72.3545311,9.19471909 L71.0963985,9.70008321 L70.4773795,9.95425044 L69.2594321,10.4655761 C56.8123589,15.7699668 48.6548341,21.7176 44.7538993,28.4742173 C37.9453585,40.2669559 44.1208291,52.9796592 61.5702618,66.1081375 L62.9900625,67.1727537 L63.8522936,67.8346365 L64.6892575,68.491774 L65.09829,68.8185721 L65.8975078,69.468645 L66.4804703,69.9531308 L66.4804703,69.9531308 L67.0493707,70.4349994 L67.7860906,71.0734406 L68.1451248,71.3909334 L68.8445933,72.0224812 C69.2263095,72.3720718 69.5965761,72.7195512 69.9554568,73.0649416 L70.4852463,73.5814631 L71.248012,74.3504059 L71.9726186,75.1123909 C78.8402712,82.495364 80.3081527,88.8974997 77.0447002,94.5499653 C74.933031,98.2074836 70.9013706,101.807999 64.958951,105.303275 L64.4134675,105.620736 L63.5293583,106.121564 L62.6189315,106.620891 C62.1058295,106.897871 61.5805462,107.174142 61.0430864,107.449677 L60.227765,107.862426 L59.2252746,108.356245 L58.1964941,108.848418 L57.1414295,109.338914 L56.6040425,109.583523 L55.5095633,110.071443 L54.3888152,110.557607 L53.2418044,111.041982 L52.6584524,111.283488 L51.4720589,111.76512 L50.869019,112.005238 L50.869019,112.005238 L50.259418,112.244884 L49.0205357,112.722748 L47.755418,113.19868 L46.4640712,113.672647 L45.1465013,114.144619 L43.8027143,114.614564 L43.1209914,114.848766 L41.7378904,115.315609 L41.0365138,115.548243 L39.6141124,116.011914 L38.1655184,116.47343 L36.3179518,117.047246 L34.4294807,117.617582 L32.8892605,118.071305 L30.739698,118.689489 C12.3072163,107.455464 0,87.1649844 0,64 C0,28.653776 28.653776,0 64,0 Z"
                                                transform="translate(.335 .835)" fill="#0080FF" />
                                        </svg>
                                        <a target="_blank" rel="noreferrer noopener"
                                            href="https://swiperjs.com/">Swiper</a>
                                    </li>
                                    <!-- AOS -->
                                    <li>
                                        <span style="color: hsl(231, 41%, 55%); font-size: 14px;">AOS</span>
                                        <a target="_blank" rel="noreferrer noopener"
                                            href="https://github.com/michalsnik/aos">AOS</a>
                                    </li>
                                </ul>
                            </div>
                            <!-- Etc -->
                            <div data-aos="fade-up" data-aos-delay="350" data-aos-once="true">
                                <span>Etc.</span>
                                <ul class="stack">
                                    <!-- Adobe Photoshop -->
                                    <li>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                            data-name="Layer 1" viewBox="0 0 256 256">
                                            <g data-name="Photo Surface">
                                                <rect width="240" height="232" x="8" y="12" fill="#001e36" rx="42">
                                                </rect>
                                            </g>
                                            <path fill="#31a8ff"
                                                d="M58 182.88V74a1 1 0 011.12-1.12C67.23 72.67 76.8 72 89.28 72c14.72 0 25.52 3.07 32.4 9.68A32.6 32.6 0 01132 106.16q0 18.09-12.8 26.88T87 141.84L78 142v40.72A1.13 1.13 0 0176.72 184H59.28C58.43 184 58 183.63 58 182.88zM78 87.62V127s4.56-.18 10-.18q10.73 0 17.36-4.72T112 106.64q0-9.44-5.84-14.56T89.28 87c-1.81 0-3.6 0-5.36.08s-3.2.08-4.32.08S77.88 87.12 78 87.62zM198 158c0 12.25-8 18-8 18v-.06c-.48.47-1 .91-1.51 1.34Q180.23 184 166.16 184t-23-5.18a1.51 1.51 0 01-1-1.63V162.12a1 1 0 01.4-.89.54.54 0 01.72.08 36.41 36.41 0 0022.24 7.78q5.59 0 8.56-2a6.15 6.15 0 003-5.27 7.72 7.72 0 00-2.4-5.75q-2.4-2.36-9.44-5.11-12.81-5-17.84-10.78A20.48 20.48 0 01142 126a22.84 22.84 0 017.84-17.21q7.51-6.81 21.44-6.81 13 0 20 3.57c.53.32.8 1 .8 2.1v13.45c0 1.3-.48 1.62-1.44 1a34.53 34.53 0 00-18.72-5.67c-3.74 0-6.48.71-8.24 2.11a6.27 6.27 0 00-2.64 5 6.9 6.9 0 002.24 5.11q2.24 2.19 9.44 5.1 12.56 4.91 18 10.32l.09.09C194.54 147.59 198 152.31 198 158z">
                                            </path>
                                        </svg>
                                        <a target="_blank" rel="noreferrer noopener"
                                            href="https://www.adobe.com/products/photoshop.html">Adobe Photoshop</a>
                                    </li>
                                    <!-- Adobe Illustrator -->
                                    <li>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                            data-name="Layer 1" viewBox="0 0 256 256">
                                            <g>
                                                <rect width="240" height="232" x="8" y="12" fill="#300" rx="42"></rect>
                                            </g>
                                            <path fill="#ff9a00"
                                                d="M128.79 156H93l-9.24 26.68Q83.6 184 82.09 184H65.29q-1.68 0-1.17-1.65L96.54 86.06A29.5 29.5 0 0097.88 77a.89.89 0 011-1H122.4c.68 0 1.07.28 1.18.82l36.28 105.7c.34 1 .06 1.48-.84 1.48H140.21a1.74 1.74 0 01-1.68-1zM97 138h27.88c-.67-1.47-13.21-41.93-14.11-45.34h-.16zM179.91 96a11.81 11.81 0 01-8.61-3.21 11.55 11.55 0 01-3.3-8.7 11.71 11.71 0 013.39-8.79 12 12 0 018.7-3.3 11.55 11.55 0 018.7 3.3A12.07 12.07 0 01192 84.09a11.55 11.55 0 01-3.3 8.7A12.07 12.07 0 01179.91 96zM170 182.5V105.34a1.18 1.18 0 011.34-1.34h17.32a1.19 1.19 0 011.34 1.34v77.32a1.18 1.18 0 01-1.34 1.34H171.5A1.33 1.33 0 01170 182.5z">
                                            </path>
                                        </svg>
                                        <a target="_blank" rel="noreferrer noopener"
                                            href="https://www.adobe.com/products/illustrator.html">Adobe Illustrator</a>
                                    </li>
                                    <!-- Boxicons -->
                                    <li>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                            viewBox="0 0 86 86">
                                            <path class="Navigation_logo_primary__St47_" fill="hsl(334, 100%, 60%)"
                                                d="M4.7,21.2c-0.9,1.4-1.4,3-1.4,4.6l0,34.8c0,2.4,1.3,4.7,3.4,5.9l31.5,17.9c2.6,1.5,5.8-0.4,5.8-3.4V43.7l0,0   L4.7,21.2z">
                                            </path>
                                            <path class="Navigation_logo_secondary__THTwH" fill="hsl(183, 76%, 54%)"
                                                d="M44,43.7l34.2-17.2c2.8-1.4,2.9-5.3,0.2-6.8L47.1,1.8c-2-1.1-4.4-1.2-6.4-0.2L8.1,18.1   c-1.4,0.7-2.6,1.8-3.5,3.1L44,43.7z">
                                            </path>
                                        </svg>
                                        <a target="_blank" rel="noreferrer noopener"
                                            href="https://boxicons.com/">Boxicons</a>
                                    </li>
                                    <!-- ICONS8 -->
                                    <li>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24">
                                            <g fill="#1FB141" clip-path="url(#a)">
                                                <path d="M12 0H0v24h12V0Z"></path>
                                                <path
                                                    d="M18 12a6 6 0 1 0 0-12 6 6 0 0 0 0 12Zm0 12a6 6 0 1 0 0-12 6 6 0 0 0 0 12Z">
                                                </path>
                                            </g>
                                        </svg>
                                        <a target="_blank" rel="noreferrer noopener"
                                            href="https://icons8.com/">ICONS8</a>
                                    </li>
                                    <!-- IconScout -->
                                    <li>
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 72 72"
                                            id="iconscout-logo">
                                            <path fill="#00c6ff" fill-rule="evenodd"
                                                d="M41.573 55.504a2.282 2.282 0 0 0-2.414-1.087 18.057 18.057 0 0 1-3.376.318c-10.29 0-18.576-8.584-18.098-18.986.427-9.289 7.96-16.828 17.242-17.255 10.396-.479 18.974 7.814 18.974 18.11 0 4.458-1.608 8.54-4.275 11.697a2.275 2.275 0 0 0-.244 2.607l5.173 8.966c.872 1.511 3.051 1.511 3.923 0L70.712 38.67a4.537 4.537 0 0 0 0-4.532L55.209 7.267A4.529 4.529 0 0 0 51.286 5H20.28a4.528 4.528 0 0 0-3.922 2.266L.855 34.136a4.535 4.535 0 0 0 0 4.533L16.358 65.54a4.528 4.528 0 0 0 3.922 2.266h24.468c1.744 0 2.833-1.888 1.962-3.4l-5.137-8.902Z"
                                                clip-rule="evenodd"></path>
                                            <path fill="#00c6ff" fill-rule="evenodd"
                                                d="M35.783 27.425c-5.003 0-9.059 4.058-9.059 9.065s4.056 9.065 9.06 9.065c5.003 0 9.058-4.058 9.058-9.065s-4.055-9.065-9.059-9.065Z"
                                                clip-rule="evenodd"></path>
                                        </svg>
                                        <a target="_blank" rel="noreferrer noopener"
                                            href="https://iconscout.com/">IconScout</a>
                                    </li>
                                    <!-- Vercel -->
                                    <li>
                                        <svg fill="#000000" width="24" height="24" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M256,48,496,464H16Z"/></svg>
                                        <a target="_blank" rel="noreferrer noopener"
                                            href="https://vercel.com/">Vercel</a>
                                    </li>
                                </ul>
                            </div>
                        </div>
                    </div>
                </div>
                <!-- What's next? -->
                <div class="whats-next">
                    <span data-aos="fade-up" data-aos-once="true">So.. What's Next?</span>
                    <p data-aos="fade-up" data-aos-delay="50" data-aos-once="true">
                        I have not much plan to do for this website, I just want to keep this website up to date with my
                        skills, recent projects, experiences, and maybe add new feature(s) in the future?
                    </p>
                </div>
            </section>
        </main>
        <footer>
            <custom-footer/>
        </footer>

        <!-- JS script link(s) -->
        <script type="text/javascript" src="js/navbar.js"></script>
        <script type="text/javascript" src="js/footer.js"></script>

        <!-- AOS Script -->
        <script type="text/javascript" src="js/aos.js"></script>
        <script type="text/javascript">
            AOS.init(); // Initialize AOS script
        </script>

        <!-- Main Padding Top Script -->
        <script type="text/javascript">
            const main_section = document.querySelector("main");

            // Wait untill document fully loaded
            document.addEventListener("DOMContentLoaded", () => {
                let navbar = document.querySelector(".navbar");

                main_section.style.paddingTop = `${navbar.scrollHeight}px`;
            });
        </script>
    </body>

</html>
